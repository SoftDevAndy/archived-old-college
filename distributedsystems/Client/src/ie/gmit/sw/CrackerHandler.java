...
public class CrackerHandler extends HttpServlet 
{
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException 
	{
		...
		if (taskNumber == null) // If the client doesn't have a task already
		{		
			jobNumber++; // Give them a job number
			
			taskNumber = new String("T" + jobNumber); // String for the user
			
			messageHandler.request(maxKeyLength,cypherText,jobNumber); // 			
		}
		else
		{
			System.out.println("Job Number: " + jobNumber);
			
			String uncyphered = messageHandler.checkRequest(jobNumber);
		...
		out.print("<H1>Processing request for Job#: " + taskNumber + " Test: " + result + "</H1>");
